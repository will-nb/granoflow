...
import 'package:granoflow/presentation/clock/clock_page.dart';
import 'package:granoflow/presentation/clock/widgets/clock_control_strip.dart';
import 'package:granoflow/presentation/clock/widgets/clock_subtask_section.dart';
...
ClockTimerState _timerState({required bool started, required bool paused}) {
...
void main() {
    ...
    await tester.pumpWidget(
      ProviderScope(
        overrides: [
          ...
          clockSubtaskListProvider.overrideWithProvider(
        ...
        child: MaterialApp(
          ...
          home: ClockPage(taskId: parentTask.id),
        ),
      ),
    );
  ...
  testWidgets('竖屏默认可见添加子任务按钮', (tester) async {
    ...
    expect(find.byType(ClockSubtaskSection), findsOneWidget);
  });
  ...
  testWidgets('横屏 800×600 使用左右分栏布局', (tester) async {
    ...
    expect(find.byType(ClockControlStrip), findsOneWidget);
  });
}
