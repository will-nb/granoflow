...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    -h|--help)
      ...
      echo ""
      echo "用法:"
      echo "  ./scripts/run_yaml_tests.sh [选项]"
      echo ""
      echo "选项:"
      ...
      echo ""
      echo "示例:"
      echo "  ./scripts/run_yaml_tests.sh                # 运行所有测试"
      echo "  ./scripts/run_yaml_tests.sh --schema       # 只运行 schema 验证"
      echo "  ./scripts/run_yaml_tests.sh --sync -v      # 运行代码同步测试（详细模式）"
      exit 0
      ;;
    *)
