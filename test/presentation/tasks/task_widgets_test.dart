import 'package:flutter/material.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';

import 'package:granoflow/generated/l10n/app_localizations.dart';
import 'package:granoflow/presentation/tasks/task_list_page.dart';
import 'package:granoflow/core/providers/app_providers.dart';
import 'package:granoflow/data/models/task.dart';

void main() {
  group('DescriptionBlock', () {
    Widget buildHarness(String description) {
      return MaterialApp(
        localizationsDelegates: AppLocalizations.localizationsDelegates,
        supportedLocales: AppLocalizations.supportedLocales,
        home: Scaffold(
          body: Center(
            child: DescriptionBlock(description: description),
          ),
        ),
      );
    }

    testWidgets('collapses long descriptions and expands on demand', (tester) async {
      final description = 'A' * 300;

      await tester.pumpWidget(buildHarness(description));
      await tester.pumpAndSettle();

      Text extractContent() {
        final candidates = tester.widgetList<Text>(find.byType(Text)).where(
          (text) => text.data != null && text.data!.contains('A'),
        );
        return candidates.first;
      }

      // Initial state should be truncated with ellipsis
      var textWidget = extractContent();
      expect(textWidget.data!.length, lessThan(description.length));
      expect(textWidget.data, endsWith('…'));
      expect(find.text('Show full description'), findsOneWidget);

      await tester.tap(find.text('Show full description'));
      await tester.pumpAndSettle();

      textWidget = extractContent();
      expect(textWidget.data, equals(description));
      expect(find.text('Show less'), findsOneWidget);
    });

    testWidgets('hides toggle for short descriptions', (tester) async {
      await tester.pumpWidget(buildHarness('Short note'));
      await tester.pumpAndSettle();

      expect(find.text('Show full description'), findsNothing);
      expect(find.text('Show less'), findsNothing);
    });
  });

  group('QuickTaskCard', () {
    testWidgets('renders execution icon for timed tasks', (tester) async {
      final task = Task(
        id: 1,
        taskId: 't-1',
        title: 'Quick timing task',
        status: TaskStatus.pending,
        createdAt: DateTime(2024, 2, 1),
        updatedAt: DateTime(2024, 2, 1),
        dueAt: DateTime(2024, 2, 2),
        tags: const <String>['#timed'],
        sortIndex: 0,
        templateLockCount: 0,
        allowInstantComplete: false,
        taskKind: TaskKind.regular,
        logs: const <TaskLogEntry>[],
      );

      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            taskTreeProvider.overrideWithProvider((taskId) {
              return StreamProvider<TaskTreeNode>((ref) {
                if (taskId == task.id) {
                  return Stream.value(
                    TaskTreeNode(task: task, children: const <TaskTreeNode>[]),
                  );
                }
                final fallback = task.copyWith(id: taskId, taskId: 'task-$taskId');
                return Stream.value(
                  TaskTreeNode(task: fallback, children: const <TaskTreeNode>[]),
                );
              });
            }),
          ],
          child: MaterialApp(
            localizationsDelegates: AppLocalizations.localizationsDelegates,
            supportedLocales: AppLocalizations.supportedLocales,
            home: Scaffold(body: QuickTaskCard(task: task)),
          ),
        ),
      );

      await tester.pump();

      expect(find.byIcon(Icons.timelapse), findsWidgets);
    });
  });
}
