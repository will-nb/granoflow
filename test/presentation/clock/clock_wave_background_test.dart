...
import 'package:granoflow/core/providers/clock_animation_providers.dart';
import 'package:granoflow/core/theme/clock_gradients.dart';
import 'package:granoflow/core/utils/gradient_composer.dart';
import 'package:granoflow/presentation/clock/widgets/clock_wave_background.dart';
...
void main() {
  ...
  testWidgets('ClockWaveBackground 在 light 模式下显示背景图片和动画层', (tester) async {
    ...
    await tester.pumpWidget(
      UncontrolledProviderScope(
        ...
        child: MaterialApp(
          ...
          home: Scaffold(
            body: ClockWaveBackground(state: ClockState.normal),
          ),
        ),
      ),
    );
  ...
  testWidgets('ClockWaveBackground 在 dark 模式下显示背景图片和动画层', (tester) async {
    final container = ProviderContainer(
      overrides: [
        waveAnimationControllerProvider.overrideWith((ref, args) {
          final config = ref.watch(waveAnimationConfigProvider(args));
