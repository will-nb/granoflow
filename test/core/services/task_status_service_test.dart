...
void main() {
  group('TaskStatusService', () {
    ...
    group('markCompleted', () {
      test(
        'should create default FocusSession when task has no running time',
        () async {
          // 创建一个没有运行时间记录的任务
          ...
          // 验证任务没有运行时间记录
          final totalMinutesBefore = await focusSessionRepository
          ...
          final totalMinutesAfter = await focusSessionRepository
              .totalMinutesForTask(task.id);
          expect(totalMinutesAfter, AppConstants.defaultTaskCompletionMinutes);

          // 验证创建的 FocusSession 详情
          final sessions = await focusSessionRepository.listRecentSessions(
            taskId: task.id,
          );
          expect(sessions.length, 1);
          expect(sessions.first.taskId, task.id);
          expect(
            sessions.first.actualMinutes,
            AppConstants.defaultTaskCompletionMinutes,
          );
          expect(
            sessions.first.estimateMinutes,
            AppConstants.defaultTaskCompletionMinutes,
          );
      ...
      test(
        'should not create FocusSession when task already has running time',
        () async {
          // 创建一个任务
          final task = await taskRepository.createTask(
            TaskDraft(title: 'Test Task', status: TaskStatus.pending),
      ...
      test(
        'should work without FocusSessionRepository (backward compatibility)',
        () async {
      ...
      test(
        ...
        () async {
          ...
          final parent = await taskRepository.createTask(
            TaskDraft(title: 'Parent Task', status: TaskStatus.pending),
          );
