...
void main() {
  group('Project Filter Providers', () {
    ...
    test('projectsForCompletedArchivedFilterProvider returns active, completed, and archived projects', () async {
      // 创建测试项目
      final projects = [
    ...
    test('projectsForTrashFilterProvider returns all projects except pseudoDeleted', () async {
      // 创建测试项目
      final projects = [
...
class _FakeProjectRepository implements ProjectRepository {
  ...
  Stream<List<Project>> watchProjectsByStatuses(Set<TaskStatus> allowedStatuses) {
    // 先发射当前值（如果有的话）
    final currentValue = _projects.where((project) {
      // 排除伪删除状态
  ...
  Future<Project?> findByIsarId(int id) async {
    return _projects.firstWhere((p) => p.id == id, orElse: () => throw Exception('Not found'));
  }
