...
import '../utils/sort_index_calculator.dart';
...
import 'tasks_section_task_tile.dart';
...
class _TasksSectionTaskListState
  ...
  Future<TaskDragIntentResult> _handleInsertionDrop(
    ...
    if (kDebugMode) {
      debugPrint(
        '[DnD] {event: _handleInsertionDrop:start, page: Tasks, section: ${section.name}, src: ${draggedTask.id}, isSubtaskPromotion: $isSubtaskPromotion, originalParentId: ${draggedTask.parentId}, targetType: $targetType, beforeTask: ${beforeTask?.id}, afterTask: ${afterTask?.id}, beforeSortIndex: ${beforeTask?.sortIndex}, afterSortIndex: ${afterTask?.sortIndex}}',
      );
    }
    try {
      ...
      if (kDebugMode) {
        if (isSubtaskPromotion && aboveTaskParentId == null) {
          debugPrint(
            '[DnD] {event: subtaskPromotion, page: Tasks, section: ${section.name}, src: ${draggedTask.id}, originalParentId: ${draggedTask.parentId}, newParentId: null (root), sortIndex: $newSortIndex, dueAt: $targetDueDate}',
          );
        }
        debugPrint(
          '[DnD] {event: call:moveToParent, page: Tasks, section: ${section.name}, src: ${draggedTask.id}, parentId: $aboveTaskParentId, sortIndex: $newSortIndex, dueAt: $targetDueDate}',
        );
      }
      ...
      if (kDebugMode) {
        debugPrint(
          '[DnD] {event: reorderTasksForSameDate:completed, page: Tasks, section: ${section.name}, targetDate: $targetDueDate}',
        );
        debugPrint(
          '[DnD] {event: accept:success, page: Tasks, section: ${section.name}, src: ${draggedTask.id}, parentId: $aboveTaskParentId, sortIndex: $newSortIndex}',
        );
      }
      ...
      if (kDebugMode) {
        debugPrint(
          '[DnD] {event: accept:error, page: Tasks, section: ${section.name}, src: ${draggedTask.id}, error: $e, stackTrace: $stackTrace}',
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    return levelMapAsync.when(
      data: (levelMap) {
        return childrenMapAsync.when(
            ...
            return Column(
              ...
              children: [
                ...
                TaskDragIntentTarget.insertion(
                  key: ValueKey('tasks-section-${widget.section.name}-insertion-first'),
                    ...
                    if (isHovering) {
                      ...
                      if (draggedTask != null) {
                        ...
                        if (movedOut) {
                          ...
                          if (kDebugMode) {
                            debugPrint(
                              '[DnD] {event: movedOutOfExpansion, page: Tasks, section: ${widget.section.name}, taskId: ${draggedTask.id}, action: hideFromUI}',
                            );
                          }
                        } else {
                          // 回到扩展区内：恢复显示
                          if (dragState.isDraggedTaskHiddenFromExpansion == true) {
                            ...
                            if (kDebugMode) {
                              debugPrint(
                                '[DnD] {event: movedBackToExpansion, page: Tasks, section: ${widget.section.name}, taskId: ${draggedTask.id}, action: showInUI}',
                              );
                            }
                          }
                        }
                    ...
                    .map((entry) {
                      ...
                      return [
                            ...
                            builder: (context) {
                              ...
                              return Container(
                                ...
                                child: TaskDragIntentTarget.surface(
                                  key: ValueKey(
                                    'tasks-section-${widget.section.name}-${task.id}',
                                  ),
                                  meta: TaskDragIntentMeta(
                                    page: 'Tasks',
                                  ...
                                  child: Builder(
                                    builder: (context) {
                                      ...
                                      return TasksSectionTaskTile(
                                        ...
                                        childWhenDraggingOpacity:
                                            isDraggedTask ? 0.0 : null,
                                        taskLevel: taskLevel,
                                        onDragStarted: () {
                                          // 使用虚拟字段 levelMap 和 childrenMap
                                          final taskLevelForDrag =
                                              levelMap[task.id] ?? 1;
                                          ...
                                          if (taskLevelForDrag == 1) {
                                            // 根任务：收缩所有子任务
                                            final childTaskIds =
                                                childrenMap[task.id] ?? <int>{};
                                            final updatedExpanded =
                                                Set<int>.from(currentExpanded);
                                        ...
                                        onDragEnd: () async {
                                          if (kDebugMode) {
                                            debugPrint(
                                              '[DnD] {event: onDragEnd:called, page: Tasks, section: ${widget.section.name}, taskId: ${task.id}}',
                                            );
                                          }
                        ...
                        if (nextTask != null)
                          TaskDragIntentTarget.insertion(
                            key: ValueKey(
                              'tasks-section-${widget.section.name}-insertion-after-${task.id}',
                            ),
                            meta: TaskDragIntentMeta(
                              page: 'Tasks',
                              ...
                              if (isHovering) {
                                ...
                                if (draggedTask != null) {
                                  ...
                                  if (movedOut) {
                                    ...
                                    // 回到扩展区内：恢复显示
                                    if (dragState.isDraggedTaskHiddenFromExpansion ==
                                        true) {
                                      dragNotifier.setDraggedTaskHidden(false);
                                      if (kDebugMode) {
                                        debugPrint(
          ...
          error: (error, stackTrace) {
            debugPrint(
              'Error loading children map for section ${widget.section.name}: $error',
            );
            return const SizedBox.shrink();
          },
        );
      ...
      error: (error, stackTrace) {
        debugPrint(
          'Error loading level map for section ${widget.section.name}: $error',
        );
        return const SizedBox.shrink();
      },
    );
