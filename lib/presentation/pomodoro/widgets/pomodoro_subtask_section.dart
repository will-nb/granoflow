...
class PomodoroSubtaskSection extends ConsumerStatefulWidget {
  const PomodoroSubtaskSection({
    super.key,
    required this.task,
  ...
  ConsumerState<PomodoroSubtaskSection> createState() =>
      _PomodoroSubtaskSectionState();
}

class _PomodoroSubtaskSectionState
    extends ConsumerState<PomodoroSubtaskSection> {
  int? _taskLevel;
  ...
  Future<void> _handleAddSubtask() async {
    ...
    // 检查是否是第一个子任务
    final childrenAsync = ref.read(pomodoroSubtaskListProvider(widget.task.id));
  ...
  void _handleSubtaskStartTimer(Task subtask) {
    // 导航到番茄时钟页面
    context.push('/pomodoro/${subtask.id}');
  }
  ...
  Widget build(BuildContext context) {
    ...
    final childrenAsync = ref.watch(
      pomodoroSubtaskListProvider(widget.task.id),
    );
